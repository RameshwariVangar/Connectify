...
const markdownContent = `# CONNECTIFY - ENTERPRISE ARCHITECTURE & TECHNICAL INTERVIEW PREPARATION GUIDE
...
async function generatePDF() {
    ...
    const pdfPath = path.join(process.cwd(), '..', 'Connectify_Enterprise_Architecture_and_Interview_Guide.pdf');
    const mdPath = path.join(process.cwd(), '..', 'Connectify_Enterprise_Architecture_and_Interview_Guide.md');

    // Save Markdown file
    fs.writeFileSync(mdPath, markdownContent, 'utf-8');
    console.log("Markdown Documentation saved at:", mdPath);
    ...
    doc.fillColor('#0f172a').fontSize(22).text('CONNECTIFY ARCHITECTURE & INTERVIEW GUIDE', { align: 'center' });
    doc.moveDown(0.3);
    doc.fillColor('#2563eb').fontSize(13).text('Enterprise Full-Stack Refactoring, Performance Isolation & Q&A Audit', { align: 'center' });
    doc.moveDown(0.5);
    ...
    for (let line of lines) {
        if (line.startsWith('# ')) {
            doc.moveDown(0.8);
            doc.fillColor('#0f172a').fontSize(16).text(line.replace('# ', '').trim());
            doc.moveDown(0.4);
        } else if (line.startsWith('## ')) {
            doc.moveDown(0.6);
            doc.fillColor('#1e40af').fontSize(14).text(line.replace('## ', '').trim());
            doc.moveDown(0.3);
        } else if (line.startsWith('### ')) {
            doc.moveDown(0.5);
            doc.fillColor('#0369a1').fontSize(12).text(line.replace('### ', '').trim());
            doc.moveDown(0.2);
        } else if (line.startsWith('* **')) {
            const cleanText = line.replace('* **', '').replace('**', ':');
            doc.fillColor('#334155').fontSize(10).text(`•  ${cleanText}`, { indent: 15 });
            doc.moveDown(0.15);
        } else if (line.startsWith('|')) {
            doc.fillColor('#1e293b').fontSize(9).text(line, { indent: 10 });
            doc.moveDown(0.1);
        } else if (line.trim().length > 0) {
            doc.fillColor('#334155').fontSize(10).text(line.trim(), { align: 'left' });
            doc.moveDown(0.25);
        }
    }
    ...
    return new Promise((resolve, reject) => {
        stream.on('finish', () => {
            console.log("PDF Generation Successfully Completed:", pdfPath);
            resolve(pdfPath);
        });
